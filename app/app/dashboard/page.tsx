  ...
  useEffect(() => {
    ...
    async function checkOnboarding() {
      try {
        ...
        // Either an API key (free tier) or a credit balance (pay-as-you-go)
        // satisfies the AI setup step.
        const hasAiAccess = Boolean(
          profile &&
            (profile.anthropic_api_key || (profile.credits_cents ?? 0) > 0)
        );
        const incomplete =
          !profile ||
          ((!hasAiAccess || !profile.master_resume_md) &&
